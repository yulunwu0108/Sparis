...
def world_to_cam(world_coords, pose, intrinsics):
    '''
    world_coords: (1, batch_size, 3)
    pose: (1, 4, 4)
    intrinsics: (1, 4, 4)
    '''

    world_coords = torch.cat((world_coords, torch.ones_like(world_coords)[..., :1]), dim=-1) # 1, batch_size, 4
    world_coords = world_coords.permute(1, 2, 0) # batch_size, 4, 1

    Rt = torch.linalg.inv(pose) # 1, 4, 4

    cam_coords = Rt[None, ...] @ world_coords # 1, batch_size, 4, 1
    K = intrinsics # 1, 4, 4 [:, :3 ,:3]
    uv_cal = K[None, ...] @ cam_coords # 1, batch_size, 4, 1

    uv_cal = uv_cal.squeeze(-1) # 1, batch_size, 4

    # import ipdb; ipdb.set_trace()

    return uv_cal[..., :2] / uv_cal[..., 2:3]
