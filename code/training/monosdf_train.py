    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                ...
                print(
                    '{0}_{1} [{2}] ({3}/{4}): loss = {5}, rgb_loss = {6}, eikonal_loss = {7}, psnr = {8}, bete={9}, alpha={10}, reproj_loss={11}, depth_loss={12}'
                        .format(self.expname, self.timestamp, epoch, data_index, self.n_batches, loss.item(),
                                loss_output['rgb_loss'].item(),
                                loss_output['eikonal_loss'].item(),
                                psnr.item(),
                                self.model.density.get_beta().item(), #module.
                                1. / self.model.density.get_beta().item(),
                                loss_output['reproj_loss'].item(),
                                loss_output['tri_depth_loss'].item(),
                                )) #module.
                ...
                self.writer.add_scalar('Statistics/beta', self.model.density.get_beta().item(), self.iter_step) # module.
                self.writer.add_scalar('Statistics/alpha', 1. / self.model.density.get_beta().item(), self.iter_step) # module.
