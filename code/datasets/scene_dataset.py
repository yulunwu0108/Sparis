...
class SceneDatasetDN(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        for idx in range(self.n_images):
            match_dict = np.load(os.path.join(self.instance_dir, 'match_roma', 'match_dict_{}.npz'.format(idx)))
            new_match_dict = {}
            new_match_dict['match_idx'] = match_dict['match_idx'].item()
            new_match_dict['kpts_B'] = torch.from_numpy(match_dict['kpts_B']).long().squeeze(0) # H*W, 2
            new_match_dict['certainty'] = torch.from_numpy(match_dict['certainty']).float().reshape(-1, 1) # H*W, 1

            new_match_dict['points_A'] = torch.from_numpy(match_dict['points_A']).float().squeeze(0) # H*W, 3
            new_match_dict['points_B'] = torch.from_numpy(match_dict['points_B']).float().squeeze(0) # H*W, 3

            self.match_dicts.append(new_match_dict)
    ...
    def __getitem__(self, idx):
        ...
        if self.sampling_idx is not None:
            ...
            if self.use_matching:
                sample["match_uv"] = self.match_dicts[idx]['kpts_B'][self.sampling_idx, :]
                sample["match_intrinsics"] = self.intrinsics_all[self.match_dicts[idx]['match_idx']]
                sample["match_pose"] = self.pose_all[self.match_dicts[idx]['match_idx']]
                sample["certainty"] = self.match_dicts[idx]['certainty'][self.sampling_idx, :]

                sample['points_main'] = self.match_dicts[idx]['points_A'][self.sampling_idx, :]
                sample['points_match'] = self.match_dicts[idx]['points_B'][self.sampling_idx, :]
