...
from kornia.geometry.epipolar import fundamental_from_projections, sampson_epipolar_distance, projection_from_KRt, symmetrical_epipolar_distance
...
class MonoSDFNetwork(nn.Module):
    ...
    def forward(self, input, indices):
        ...
        main_depth_values = depth_values

        ''' calculate surface normals '''
        surface_points = cam_loc + depth_values * ray_dirs
        ...
        if 'match_uv' in input:
            with torch.no_grad():
                match_uv = input['match_uv'].cuda()
                match_intrinsics = input['match_intrinsics'].cuda()
                match_pose = input['match_pose'].cuda()
                main_points_tri = input['points_main'].cuda()
                match_points_tri = input['points_match'].cuda()

            match_ray_dirs, match_cam_loc = rend_util.get_camera_params(match_uv, match_pose, match_intrinsics)

            _, num_pixels, _ = match_ray_dirs.shape

            match_cam_loc = match_cam_loc.unsqueeze(1).repeat(1, num_pixels, 1).reshape(-1, 3)
            match_ray_dirs = match_ray_dirs.reshape(-1, 3)

            match_z_vals, match_z_samples_eik = self.ray_sampler.get_z_vals(match_ray_dirs, match_cam_loc, self)
            # N_samples = z_vals.shape[1]

            match_points = match_cam_loc.unsqueeze(1) + match_z_vals.unsqueeze(2) * match_ray_dirs.unsqueeze(1)
            match_points_flat = match_points.reshape(-1, 3)

            match_sdf, _, _ = self.implicit_network.get_outputs(match_points_flat)

            # rgb_flat = self.rendering_network(points_flat, gradients, dirs_flat, feature_vectors, indices)
            # rgb = rgb_flat.reshape(-1, N_samples, 3)

            match_weights = self.volume_rendering(match_z_vals, match_sdf)

            # rgb_values = torch.sum(weights.unsqueeze(-1) * rgb, 1)

            match_depth_values = torch.sum(match_weights * match_z_vals, 1, keepdims=True) / (match_weights.sum(dim=1, keepdims=True) +1e-8)

            ''' calculate surface normals '''
            match_surface_points = match_cam_loc + match_depth_values * match_ray_dirs # batch_size, 3

            main2match_uv = rend_util.world_to_cam(surface_points.unsqueeze(0), match_pose, match_intrinsics)
            match2main_uv = rend_util.world_to_cam(match_surface_points.unsqueeze(0), pose, intrinsics)

            ''' sampson weight & reprojection sampson dist '''
            with torch.no_grad():
                P_main = projection_from_KRt(intrinsics[:, :3, :3], torch.linalg.inv(pose)[:, :3, :3], torch.linalg.inv(pose)[:, :3, 3:])
                P_match = projection_from_KRt(match_intrinsics[:, :3, :3], torch.linalg.inv(match_pose)[:, :3, :3], torch.linalg.inv(match_pose)[:, :3, 3:])

                Fm = fundamental_from_projections(P_main, P_match)
                sampson_dist = symmetrical_epipolar_distance(uv.float(),
                                                            match_uv.float(),
                                                            Fm,
                                                            squared=False,
                                                            )
                sampson_weight = (1 - torch.sigmoid(0.1 * sampson_dist)) * 2 # (1. + torch.exp(-sampson_dist)) / 2. # 1, batch_size
                # print(sampson_dist, sampson_weight.mean())

            # reproj_sampson_dist = symmetrical_epipolar_distance(match2main_uv.float(),
            #                                                     main2match_uv.float(),
            #                                                     Fm,
            #                                                     squared=False,
            #                                                     )
            output['sampson_weight'] = sampson_weight
            # output['reproj_sampson_dist'] = reproj_sampson_dist

            ''' reprojection error '''
            uv_diff_norm_main = torch.norm(match2main_uv - uv, dim=-1)
            uv_diff_norm_match = torch.norm(main2match_uv - match_uv, dim=-1)

            uv_diff_norm = uv_diff_norm_main + uv_diff_norm_match # 1, batch_size

            ''' triangulation depth loss '''
            main_depth_tri = torch.norm(main_points_tri - cam_loc, dim=-1) # 1, batch_size
            match_depth_tri = torch.norm(match_points_tri - match_cam_loc, dim=-1) # 1, batch_size

            main_depth_values = main_depth_values.reshape_as(main_depth_tri)
            match_depth_values = match_depth_values.reshape_as(match_depth_tri)

            tri_depth_values = torch.abs(main_depth_values / main_depth_tri - 1) + \
                                  torch.abs(match_depth_values / match_depth_tri - 1)

            output['uv_diff_norm'] = uv_diff_norm
            output['certainty'] = input['certainty']
            output['tri_depth_values'] = tri_depth_values

            output['tri_depth_z'] = depth_scale * main_depth_values.t()

            # points_tri_norm = torch.norm(main_points_tri - match_points_tri, dim=-1)
            # points_tri_norm.requires_grad_(False)

            # output['surface_points_norm'] = torch.norm(surface_points - match_surface_points, dim=-1).unsqueeze(0) # 1, batch_size
